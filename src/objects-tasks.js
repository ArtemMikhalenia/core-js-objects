...
function mergeObjects(objects) {
  const newObject = {};
  const result = {};
  Object.assign(newObject, objects);
  Object.values(newObject).reduce((acc, obj) => {
...
function makeWord(lettersObject) {
  const resultLength = Math.max(...[].concat(...Object.values(lettersObject)));
  const result = new Array(resultLength > 0 ? resultLength + 1 : 1).fill('');

  Object.entries(lettersObject).forEach(([key, value]) => {
    value.forEach(() => {
      for (let i = 0; i < value.length; i += 1) {
        result.splice(value[i], 1, key);
      }
    });
  });
...
function sellTickets(queue) {
  const result = queue.reduce((change, payment) => {
    if (payment === 25) {
...
const cssSelectorBuilder = {
  ...
  element(value) {
    this.checkOrder(1);
    return this.createNewObj(value, 1);
  },

  id(value) {
    this.checkOrder(2);
    return this.createNewObj(`#${value}`, 2);
  },

  class(value) {
    this.checkOrder(3);
    return this.createNewObj(`.${value}`, 3);
  },

  attr(value) {
    this.checkOrder(4);
    return this.createNewObj(`[${value}]`, 4);
  },

  pseudoClass(value) {
    this.checkOrder(5);
    return this.createNewObj(`:${value}`, 5);
  },

  pseudoElement(value) {
    this.checkOrder(6);
    return this.createNewObj(`::${value}`, 6);
  },

  combine(selector1, combinator, selector2) {
    const obj = Object.create(this);
    obj.selectors = `${selector1.selectors} ${combinator} ${selector2.selectors}`;
    return obj;
  },

  checkOrder(num) {
    if (this.order === num && this.orderArray.includes(num)) {
      throw new Error(
